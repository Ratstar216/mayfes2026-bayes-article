...
他の記事は難しそうなのですが, この記事は高校生でも理解できる説明を目指します(というつもりで書いたのですが, 行列を回避できませんでした...). 
\vspace{-0.5em}
\subsection{回帰}
...
さらに, この後新しくデータを観測することができて, 合計 100 個のデータを使えるようになったとしましょう. その場合, 以下のようになります. 
\begin{figure}[H]
  \centering
...
\subsection{おわりに}
ベイズ線形回帰について説明しました. 頻度論的なアプローチに対して, ベイズ的な考え方は不確実性を上手く扱えるという長所がありますが, 一方で短所もあります. その一つは, 事前分布の仮定です. 何も仮定しなくて良い最小二乗法と比べて, 
今回扱ったベイズ線形回帰は事前分布を仮定しました. この仮定はそれなりに強い仮定で, 必ずしも上手くいくとは限りません. 
この記事を書くにあたって,  \cite{grosse2018bayesianlinearregression}, \cite{suyama2019bayesianDeepLearning} を参考にしました. また、可視化などのためにここで用いたコードは、\url{https://github.com/Ratstar216/mayfes2026-bayes-article} で後悔しています。
